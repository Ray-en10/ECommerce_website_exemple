...
function Sign() {
  //sing up
    ...
    const submitForm=(e)=>{
      ...
      if(data.name.length === 0 || data.email.length === 0 || (data.password && data.password.length >= 4))
      {
        const sendData = {
                name:data.name,
                email: data.email,
            ...
            axios.post('http://localhost/php-react/login-php/insert.php', sendData) 
            .then((result) =>{ if (result.data.Status === 'Invalid') 
            { alert('Invalid User'); } 
            else  {
...
const handleSubmit = (e) => {
  ...
  if (name === "admin" && password === "admin12") {
    ...
    fetch("http://localhost/php-react/login-php/compare.php", {
      method: "post",
      headers: {
